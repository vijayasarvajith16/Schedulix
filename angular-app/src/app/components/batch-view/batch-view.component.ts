...
export class BatchViewComponent implements OnInit, OnDestroy {
    ...
    loadBatches(): void {
        ...
        this.batchSub = this.timetableService.getBatches()
            ...
            .subscribe({
                ...
                error: () => {
                    this.error = 'Error loading batches. Please refresh.';
                    this.loadingParams = false;
                }
            });
    }
    ...
    onBatchSelect(): void {
        const batch = this.batches.find(b => b._id === this.selectedBatch);
    ...
    getColor(slot: TimetableSlot | undefined): string {
        if (!slot || !slot.subjectId) return 'transparent';
    ...
    exportCSV(): void {
        ...
        const header = ['Period', ...this.DAYS].join(',');
        const rows = this.PERIODS.map(p => {
            const cells = this.DAYS.map(d => {
                const slot = this.getSlot(d, p);
                return slot ? `"${slot.subjectId?.subjectName || ''}"` : '""';
            });
            return [`Period ${p}`, ...cells].join(',');
        });
        ...
        a.href = url;
        a.download = `timetable_${this.selectedBatchName || 'batch'}.csv`;
        a.click();
